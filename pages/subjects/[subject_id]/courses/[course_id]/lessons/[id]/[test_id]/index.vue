<template>
  ...
  <main>
    ...
    />
    <form
      v-if="$router.currentRoute.value.name == 'subjects-subject_id-courses-course_id-lessons-id-test_id' && $router.currentRoute.value.query.type == 'grouptest'"
    ...
    >
      <div class="space-y-2 sm:w-[400px] w-[80%]">
        ...
        <input type="text" placeholder="Ismingizni kiriting" id="username" required />
      </div>
      <div class="space-y-2 sm:w-[400px] w-[80%]">
        <label for="code">Test kodi</label>
        <input type="number" placeholder="Test kodini kiriting" id="code" required />
      </div>
      <button class="login_btn mx-auto">Davom etish</button>
    </form>
    <div v-else>
      <nav>
...
onBeforeMount(() => {
  if (router.currentRoute.value.query.type == "grouptest") {
    return;
  }
  useTest.getTests();
});
