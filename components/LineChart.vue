...
//   const nextMonth = new Date(year, useProduct.state.chart_month + 1, 1);
//   const lastDateOfMay = new Date(nextMonth - 1).getDate();

//   const dates = [];
//   for (let i = 1; i <= lastDateOfMay + 1; i++) {
//     const date = new Date(store.year, useProduct.state.chart_month, i);
//     dates.push(date);
//   }

//   const token = localStorage.getItem("token");
//   is_mount.value = false;

//   axios
//     .post(
//       baseUrl + "/orders/sales_orders/analysis/" + useProfile.store.id,
//       {
//         dates: dates,
//       },
//       {
...
//         "--------------------------------------------------------"
//       );
//       store.datas = Object.values(res.data?.data).slice(1, -1);
//       props.series[0].data = [];
//       props.series[1].data = [];
//       for (let i of store.datas) {
//         props.series[0].data.push(i.totalAmount);
//         props.series[1].data.push(i.ordersCount);
//       }
//       is_mount.value = true;
//       useProduct.state.totla_month_salary = props.series[0].data.reduce(
...
//       console.log(err);
//     });
//   console.log("Last Date of May:", lastDateOfMay);
//   useProduct.state.lastDate = lastDateOfMay;
//   props.chartOptions.xaxis.categories = [];
//   for (let i = 0; i < lastDateOfMay; i++) {
//     console.log(i);
//     props.chartOptions.xaxis.categories.push(
//       `${i + 1} ${useProduct.state.month_name}`
//     );
//   }
// }
          ...
          return `
            <div class="relative">
              ...
              <div class="absolute top-0 left-0 space-y-[5px] -ml-[150px] -mt-16 w-[162px] py-[15px] overflow-hidden text-center">
                <h1 class="text-xs text-[#088178] font-medium">${series[1][dataPointIndex]}+ buyurtma</h1>  
...
watch(
  () => useProfile.store.is_activity,
  () => {
    if (useProfile.store.is_activity) {
      props.series[0].data = []
      for (let i of useProfile.store.currentStep) {
        props.series[0].data.push(i[3] ? i[3] : 0);
      }
      console.log(props.series[0].data);
      // props.series[0].data = useProfile.store.currentStep;
      is_mount.value = true;
    }
  }
);
